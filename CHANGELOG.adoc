...
- https://github.com/eclipse-sirius/sirius-web/issues/5301[#5301] [core] Fix browser history on workbenches
...
- https://github.com/eclipse-sirius/sirius-web/issues/6521[#6521] [sirius-web] Add the ability to contribute additional GraphQL instrumentations
...
- Add support for node decorators
- Add support for diagram minimap default visibility
- Allow users to filter a given selection on diagrams
...
- [ADR-228] Add auto alignment for edges
...
- https://github.com/eclipse-sirius/sirius-web/issues/6331[#6331] [sirius-web] Clarify our logging policy in a document available in `doc/how-to/logging-policy.adoc`
- https://github.com/eclipse-sirius/sirius-web/issues/6250[#6250] [selection] Update the GraphQL API of `SelectionDescription` type.
- https://github.com/eclipse-sirius/sirius-web/issues/6262[#6262] [core] The method `ICustomNodeStyleDeserializer#handle(...)` has been modified to `INodeStyle handle(ObjectMapper mapper, String root) throws JsonProcessingException;`
The method `IRepresentationDeserializer#handle(...)` has been modified to `Optional<IRepresentation> handle(JsonParser jsonParser, DeserializationContext context, ObjectNode root);`
...
Custom deserializers and migration services will probably be impacted.
- https://github.com/eclipse-sirius/sirius-web/issues/6369[#6369] [diagram] In order to click behind the rectangle selection on a diagram, you'll need to update `ReactFlow.css` as per the `selection rectangle` section.
-https://github.com/eclipse-sirius/sirius-web/issues/6067[#6067] [diagram] `moving` should be set when converting custom nodes.
- https://github.com/eclipse-sirius/sirius-web/issues/6243[#6243] [sirius-web] Add documentation in the new representation modal.
`RepresentationDescriptionMetadata` has been renamed to `RepresentationMetadataDTO` and moved to `org.eclipse.sirius.components.collaborative.dto`.
It now requires a `documentation` parameter.
- https://github.com/eclipse-sirius/sirius-web/issues/6451[#6451] [tree] `ISingleClickTreeItemContextMenuEntryExecutor.canExecute` method has now more parameters: `boolean canExecute(IEditingContext editingContext, TreeDescription treeDescription, String treeItemMenuContextEntryId, ITreeInput treeInput);`


=== Dependency update

- [releng] Switch to lexical 0.42.0
- https://github.com/eclipse-sirius/sirius-web/issues/6299[#6299] [releng] Switch to https://vite.dev/blog/announcing-vite8[Vite 8.0.1] and @vitejs/plugin-react 6.0.1
- [releng] Switch to https://github.com/spring-projects/spring-boot/releases/tag/v4.0.6[Spring Boot 4.0.6]
- [releng] Switch to `jacoco-maven-plugin` 0.8.14
- [releng] Switch to `com.tngtech.archunit:archunit-junit5` 1.4.1
- [releng] Switch to CheckStyle 13.3.0.
Note this version requires at least Java 21 for the build environment (Sirius Web itself still only requires Java 17 to run).
...
- https://github.com/eclipse-sirius/sirius-web/issues/6320[#6320] [diagram] Prevent inside label to be centered regardless of the configured position
...
- https://github.com/eclipse-sirius/sirius-web/issues/6309[#6309] [diagram] Add support for multi-select node resizing with position adjustments
- https://github.com/eclipse-sirius/sirius-web/issues/6386[#6386] [6386] Fix an issue where the handles of an edge on edge could be incorrect
- https://github.com/eclipse-sirius/sirius-web/issues/6387[#6387] [diagram] Set the initial position of an EdgeAnchorNode when converting the diagram
- https://github.com/eclipse-sirius/sirius-web/issues/6155[#6155] [diagram] Prevent edge handle detachment after node resize
- https://github.com/eclipse-sirius/sirius-web/issues/6340[#6340] [diagram] Correctly reset mouse cursor after a failed new connection
- https://github.com/eclipse-sirius/sirius-web/issues/6443[#6443] [selection] Fix the selection dialog confirm button label emptiness when the selection is required.
- https://github.com/eclipse-sirius/sirius-web/issues/6441[#6441] [diagram] Prevent helper lines from staying visible after node resize
- https://github.com/eclipse-sirius/sirius-web/issues/6437[#6437] [diagram] Prevent list node layout to be lost during node resize
- https://github.com/eclipse-sirius/sirius-web/issues/6436[#6436] [core] Fix size of panels configured to be collapsed by default
- https://github.com/eclipse-sirius/sirius-web/issues/5983[#5983] [diagram] Prevent a valid drop candidate from randomly remaining faded
- https://github.com/eclipse-sirius/sirius-web/issues/6451[#6451] [tree] Fix an issue where the wrong `ISingleClickTreeItemContextMenuEntryExecutor` could be selected
- https://github.com/eclipse-sirius/sirius-web/issues/6482[#6482] [diagram] Fix an issue where the handles of an edge on edge could have the wrong position
- https://github.com/eclipse-sirius/sirius-web/issues/6481[#6481] [core] Fix renaming in the views panel where the text field was not displaying the current value.
- https://github.com/eclipse-sirius/sirius-web/issues/6476[#6476] [diagram] Prevent the background color from always being black in full-screen mode
- https://github.com/eclipse-sirius/sirius-web/issues/6474[#6474] Move tooltip on explorer items to avoid clicking on it during explorer navigation.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6377[#6377] [diagram] Add frontend support for rendering node decorators.
Decorators can be placed on any of the cardinal and ordinal directions (north, north west, etc) or at the center position.
Sirius Web provides and uses a `DecoratorContainer` that can render these decorators at the appropriate position.
Downstream applications can either reuse this container, or manually handle the `NodeData#decorators` in their renderers to adapt decorator rendering to their custom nodes.
- https://github.com/eclipse-sirius/sirius-web/issues/6383[#63830] [diagram] Display all the decorators on a given position in a tooltip.
- https://github.com/eclipse-sirius/sirius-web/issues/6406[#6406] [diagram] Add backend support for node decorators.
Specifiers can now define `NodeDecorators` as part of their `DiagramDescription`.
A node decorator references the `NodeDescriptions` on which it can be applied, and has a `preconditionExpression` to further detail when it should be displayed.
Decorators also have an `iconURLExpression` and `labelExpression` to specify the decorator's icon and the message to display in the tooltip when hovering it.
- https://github.com/eclipse-sirius/sirius-web/issues/6410[#6410] [diagram] Add support for semantic decorators in diagrams.
Specifiers can now use `SemanticDecoratorDescription` to define decorators that are applied on nodes representing a given domain type.
The decorator also accepts a `preconditionExpression` to further detail when it should be displayed.
- https://github.com/eclipse-sirius/sirius-web/issues/6412[#6412] [diagram] Support the placement of edges on edges along the same edge with individual positions.
- https://github.com/eclipse-sirius/sirius-web/issues/6469[#6469] [diagram] Add orthogonal layout configuration
...
- https://github.com/eclipse-sirius/sirius-web/issues/6331[#6331] [sirius-web] Improve the log level of existing log messages to align them with our logging policy.
Switch all usage of the logger to the fluent API to prepare for the addition of more data.
Improve the log message used when logging exceptions.
The log level of the registration of the GraphQL schema on startup has been changed from `INFO` to `DEBUG`.
Add log messages for the access to the main parts of the application (project templates, project, libraries, editing context, subscriptions).
Add log messages for the manipulation of projects
- https://github.com/eclipse-sirius/sirius-web/issues/6366[#6366] [diagram] Extract layout tools in several hooks.
- https://github.com/eclipse-sirius/sirius-web/issues/6250[#6250] [selection] Move the selection dialog default settings to the backend
...
- https://github.com/eclipse-sirius/sirius-web/issues/6248[#6248] [view] Add an `endUserDocumentation` field in `RepresentationDescription`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6370[#6370] [diagram] Set the xyflow multi selection behavior to `Partial mode`
- https://github.com/eclipse-sirius/sirius-web/issues/6369[#6369] [diagram] Enable clicking behind the multiple-selection rectangle
- https://github.com/eclipse-sirius/sirius-web/issues/6300[#6300] [diagram] Make the arrange-all button remember the last layout configuration used.
- https://github.com/eclipse-sirius/sirius-web/issues/6067[#6067] [diagram] Preserve edge path when moving both source and target
- https://github.com/eclipse-sirius/sirius-web/issues/6428[#6428] [forms] The existing `widgetContributionExtensionPoint` frontend extension point can now be used to override the frontend implementation of a widget with a custom React component (which only needs to implement `PropertySectionComponent<GQLWidget>`).
See ADR-230 for the details.
- https://github.com/eclipse-sirius/sirius-web/issues/6243[#6243] [sirius-web] Add documentation in the new representation modal.
- https://github.com/eclipse-sirius/sirius-web/issues/6349[#6349] [diagram] Prevent minor misalignment on edges by align edge source and target positions.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6421[#6421] [diagram] Add support for handling edge bend points during the `arrange all` action
- https://github.com/eclipse-sirius/sirius-web/issues/6422[#6422] [diagram] Add support for handling edge handles during the `arrange all` action
- https://github.com/eclipse-sirius/sirius-web/issues/6236[#6236] [sirius-web] Add "select" and "filter" tools to the "views explorer" view.
- https://github.com/eclipse-sirius/sirius-web/issues/6449[#6449] [tree] The backend tree rendering now filters out empty fragments in item's labels to reduce the amount of data transfered to the frontend.
- https://github.com/eclipse-sirius/sirius-web/issues/6396[#6396] [diagram] The diagram palette now supports fuzzy matching when searching for tools.
For example entering either `att def` or `NAD` will now find a tool named `New Attribute Definition`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6194[#6194] [core] GraphQL requests should stop using `ErrorPayload#message` and rely instead only on `ErrorPayload#messages`


=== Breaking changes

- https://github.com/eclipse-sirius/sirius-web/issues/6120[#6120] [sirius-web] The service `IProjectApplicationService` has been deleted and replaced with `IProjectSearchApplicationService`, `IProjectCreationApplicationService`, `IProjectUpdateApplicationService`, `IProjectDeletionApplicationService`.
- https://github.com/eclipse-sirius/sirius-web/issues/6039[#6039] [sirius-web] Keep active tab open in the details view on selection change.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6146[#6146] [sirius-web] Add an option to search in libraries in the search view.
`SearchQuery` now requires a new `searchInLibraries` parameter.
The GraphQL schema has been updated to reflect this change.
- https://github.com/eclipse-sirius/sirius-web/issues/6157[#6157] [diagram] On the frontend, `DiagramPanelActionProps` and `diagramPanelActionExtensionPoint` have been renamed to `DiagramToolbarActionProps` and `diagramToolbarActionExtensionPoint`.
- https://github.com/eclipse-sirius/sirius-web/issues/4163[#4163] [sirius-web] Allow external resource loaders to return reports on resource upload.
`IExternalResourceLoaderService#getResource` now returns a `ExternalResourceLoadingResult`.
`SanitizedResult` now requires an `ILoadingReport` argument.
`UploadedResource` now requires an `ILoadingReport` argument.
`IUploadDocumentReportProvider#canHandle` and `#createReport` now take an `UploadedResource` instead of a `Resource` as parameter.
- https://github.com/eclipse-sirius/sirius-web/issues/6157[#6157] [diagram] Every diagram that does not declare a toolbar will not have a toolbar anymore.
A toolbar will be added by a migration participant for existing diagram descriptions.
- https://github.com/eclipse-sirius/sirius-web/issues/6196[#6196] [selection] The `SelectionDescription` builder now requires a `noSelectionLabelProvider` to build.
- https://github.com/eclipse-sirius/sirius-web/issues/5941[#5941] [tree] Custom tree descriptions will not have any tooltip by default.
The tooltip is now configurable using either the view based tree description or using an instance of `ITreeItemTooltipProvider`.
The default tooltip used in the explorer can be reused thanks to `IExplorerTooltipService` and it can be customized for any tree item using `IExplorerTooltipServiceDelegate`.
- https://github.com/eclipse-sirius/sirius-web/issues/6057[#6057] [diagram] During node converter, `InsideLabel` must declare a width and height, for that `convertInsideLabel` changed.
An example of this impact could be found in `EllipseNodeConverter.ts`.
In the node layout, we no longer need to retrieve the label element, as the size is now present in the label data.
This means that the following methods `getHeaderHeightFootprint` and `getInsideLabelWidthConstraint` have changed their signature by removing the label element.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6145[#6145] [diagram] Fix "leaf" node resize lower than its default size.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6220[#6220] [diagram] Prevent keyboard shortcuts from being ignored when the diagram has the focus.
- https://github.com/eclipse-sirius/sirius-web/issues/5988[#5988] [sirius-web] Improve resize for panels: the resize cursor is now displayed on the whole hit area rather than only over the separator.
- https://github.com/eclipse-sirius/sirius-web/issues/6192[#6192] [sirius-web] Fix deadlock when importing a project with a representation having an unknown representation description ID.
- https://github.com/eclipse-sirius/sirius-web/issues/6184[#6184] [diagram] Prevent manually moved edge labels to change position after resize
- https://github.com/eclipse-sirius/sirius-web/issues/6223[#6223] [diagram] Add dynamic edge selection area with zoom responsiveness
- https://github.com/eclipse-sirius/sirius-web/issues/6285[#6285] [diagram] Prevent hidden edges to interfere with border node position
- https://github.com/eclipse-sirius/sirius-web/issues/6259[#6259] [view] Fix icon of the diagram toolbar that was not displayed in the View DSL.
- https://github.com/eclipse-sirius/sirius-web/issues/6260[#6260] [diagram] Prevent unselected nodes from being resized
- https://github.com/eclipse-sirius/sirius-web/issues/6202[#6202] [diagram] Prevent compatible nodes from being highlighted when dragging a border node
- https://github.com/eclipse-sirius/sirius-web/issues/6258[#6258] [diagram] Prevent child nodes from detaching during parent node resize
- https://github.com/eclipse-sirius/sirius-web/issues/6138[#6138] [diagram] Prevent handle position to move between two refreshes
- https://github.com/eclipse-sirius/sirius-web/issues/6306[#6306] [diagram] Apply move when resizing a child node from its left side
- https://github.com/eclipse-sirius/sirius-web/issues/6294[#6294] [table] Prevent redundant `getAllToolMenuEntries` calls for each action


=== New Features

- https://github.com/eclipse-sirius/sirius-web/issues/2503[#2503] Add a dedicated documentation field in the View model
  * In the View model, added a `description` attribute (documentation) on multiple model elements (diagram and view models: diagram element descriptions, node/edge style descriptions, tools, etc.).
- https://github.com/eclipse-sirius/sirius-web/issues/6151[#6151] [diagram] Allow the resize of several elements during multi selection
- https://github.com/eclipse-sirius/sirius-web/issues/6157[#6157] [diagram] Add a new API endpoint to enable or disable the diagram toolbar.
- https://github.com/eclipse-sirius/sirius-web/issues/6157[#6157] [diagram] Add diagram toolbar to the View DSL.
If present, the toolbar will be enabled, disabled otherwise.
- https://github.com/eclipse-sirius/sirius-web/issues/6196[#6197] [view] Update the view metamodel to make the selection dialog optional.
- https://github.com/eclipse-sirius/sirius-web/issues/3459[#3459] [diagram] Add support for edges on `IconLabel` nodes
- https://github.com/eclipse-sirius/sirius-web/issues/6157[#6157] [diagram] Add ability to collapse/expand the diagram toolbar.
- https://github.com/eclipse-sirius/sirius-web/issues/6203[#6203] [selection] Update the selection dialog according to the changes made in the selection dialog description.
- https://github.com/eclipse-sirius/sirius-web/issues/1626[#1626] [diagram] Add the ability to trigger tools using key bindings
It is now possible to add `KeyBinding` instances to node tools in a diagram (see `NoteToolBuilder#keyBindings`), and specify the keys to press to trigger the tool.
The key bindings that can be triggered at a given time are computed from the palette of the selected diagram elements.
For example, the tools contained in a given `NodeDescription` can only be triggered via key bindings if the selected diagram element is a node with the given `NodeDescription`.
...
For the moment, it is recommended not to use key combinations which can conflict with existing browser actions (e.g. avoid using _Ctrl-d_ for a diagram tool, as it is a standard in most browsers to create a new bookmark).
...
- https://github.com/eclipse-sirius/sirius-web/issues/6105[#6105] [sirius-web] Publishing a studio now publishes its representations.
- https://github.com/eclipse-sirius/sirius-web/issues/6039[#6039] [sirius-web] Keep active tab open in the details view on selection change.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6146[#6146] [sirius-web] Add an option to search in libraries in the search view.
- https://github.com/eclipse-sirius/sirius-web/issues/6173[#6173] Add the hook `useCurrentLibrary` to access the current library in the library workbench.
- https://github.com/eclipse-sirius/sirius-web/issues/6129[#6129] [diagram] Added support for _Show in_ actions from multi selection on diagrams
- https://github.com/eclipse-sirius/sirius-web/issues/6148[#6148] [sirius-web] Add an extension point to contribute actions to the query result button.
The contribution should provide the `label` to display in the split button menu as well as the component to render when the action is selected in the menu.
- https://github.com/eclipse-sirius/sirius-web/issues/5888[#5888] [sirius-web] The _Search_ now remembers the last query parameters used on a per-project basis
- https://github.com/eclipse-sirius/sirius-web/issues/4163[#4163] [sirius-web] Allow external resource loaders to return reports on resource upload.
Implementations of `IExternalResourceLoaderService` now return an `ExternalResourceLoadingResult` which contains an `ILoadingReport`.
`LoadingReport` instances are automatically handled by Sirius Web and displayed to the end user.
Alternative implementations of `ILoadingReport` can be provided and consumed by a matching `IUploadDocumentReportProvider` implementation.
- https://github.com/eclipse-sirius/sirius-web/issues/5899[#5899] [table] Add the possibility to retrieve table tool menu entries from backend
- https://github.com/eclipse-sirius/sirius-web/issues/5941[#5941] [tree] Add support for customizing tree item tooltips.
It is now possible to implement `ITreeItemTooltipProvider` to provide tooltips for any tree.
Trees defined via the view DSL can alternatively provide a tooltip expression via `TreeDescription#setTreeItemTooltipExpression`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6178[#6178] [diagram] Prevent edge label from sticking to the edge path
...
- [diagram] Finding the variables accessible for a given operation is a complex task today.
The name of the various variables are scattered all across the codebase without any consistency.
To fix this issue, the various variables used will be consolidated in dedicated classes such as `CommonVariables` and `DiagramVariables`.
These variables will be used along with the definition of various operations (i.e. `DiagramRenderingOperations` or `DiagramInteractionOperations`) to document and check the variable accessible for a given operation (see `DiagramVariableProvider`).
It will be possible to validate the variables used for an operation using `IOperationValidator`.
As a result, all the existing constants used to store variable names will be progressively deprecated since they cannot be found easily and lack some meaningful information like the type and documentation.
Calls to constants like `VariableManager.SELF` or `IEditingContext.EDITING_CONTEXT` will be progressively replaced by using `CommonVariables`.
Diagram specific constants like `Node.SELECTED_NODE` will be replaced by using `DiagramVariables`.
Other representations will follow the same patterns.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5714[#5714] The `createProjectFromTemplate` mutation and all associated types (e.g. `CreateProjectFromTemplateInput` and `CreateProjectFromTemplateSuccessPayload`) and test helpers (`CreateProjectFromTemplateMutationRunner`) have been removed.
...
This responsibility has been transferred to the semantic data initializer in order to offer the possibility to provide a custom cause for the persistence of the semantic data.
...
This API will probably slightly evolve in the future but it provides today the ability to change the project created from the default "Blank Project".
...
- https://github.com/eclipse-sirius/sirius-web/issues/5585[#5585] [sirius-web] The foreign keys `fk_project_semantic_data_project_id` and `fk_project_semantic_data_semantic_data_id` have been removed.
They should not have existed at all since we should not have any foreign keys between bounded contexts.
Their existence allowed us to clean some part of the database "for free" thanks to some "ON DELETE CASCADE" instructions.
They have been replaced by proper transactional event listeners and new events such as `ProjectSemanticDataDeletedEvent` and `SemanticDataDeletedEvent`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5884[#5884] [diagram] The `ReactFlow.css` has been updated to reduce the size of edge handles.
- https://github.com/eclipse-sirius/sirius-web/issues/5934[#5934] [diagram] All mutations related to the update of the appearance of an edge or node (`EditxxxAppearanceInput` && `ResetxxxAppearanceInput`) have been modified to take a list of elements instead of only one.
- https://github.com/eclipse-sirius/sirius-web/issues/6000[#6000] [test] Add the ability to test errors more easily.
The API of `IGraphQLRequestor` has thus been updated to stop returning only the data of the GraphQL response but to also include its errors and extensions.
On top of that, the `IGraphQLRequestor` will stop considering that a GraphQL response containing some errors should require the test to fail.
As a result, the methods of `IGraphQLRequestor` like `String execute(String query, Map<String, Object> variables)` have been changed to `GraphQLResult execute(String query, Map<String, Object> variables)`.
The `GraphQLResult` or `GraphQLSubscriptionResult` (in case of a subscription) is a simple record containing the data, errors and extensions.
Core test services relying on this API have thus been modified to return the same `GraphQLResult` or `GraphQLSubscriptionResult` such as `IGivenCreated{Deck|Diagram|FormDescriptionEditor|Form|Gantt|Portal|Table|Tree}Subscription`.
In order to be leveragable by most tests, this change also impact the API of `IQueryRunner`, `IMutationRunner` and `ISubscriptionRunner`.
For tests with calls such as `this.givenCreatedDiagramSubscription.createAndSubscribe(input)`, the change will require you to ask for the flux from the response by using `this.givenCreatedDiagramSubscription.createAndSubscribe(input).flux()`.
For tests where the result of a query or mutation is parsed using `JsonPath`, the main change should be switching from `JsonPath.read(result, "$.data.viewer...")` to `JsonPath.read(result.data(), "$.data.viewer...")`.
- Remove `IGivenCommittedTransaction` since it has been replaced a long time ago by a proper initialization of our test state using `@GivenSiriusWebServer`.
Downstream projects relying on this should switch to a better initialization or their test or use `TestTransaction` directly.
- https://github.com/eclipse-sirius/sirius-web/issues/5765[#5765] [diagram] The method `INodeStyleProvider#createNodeStyle` has now access to both the AQL interpreter and the variable manager to compute expressions.
The optional editing context id argument has been removed since it can be computed from the variable manager.
- https://github.com/eclipse-sirius/sirius-web/issues/4860[#4860] [core] The `workbenchViewContributionExtensionPoint` frontend extension point has slightly changed since the `side` attribute has been removed and it is now managed directly by the workbench configuration.
- https://github.com/eclipse-sirius/sirius-web/issues/5815[#5815] [diagram] `isLastNodeSelected` should be set when converting custom nodes.
- https://github.com/eclipse-sirius/sirius-web/issues/5585[#5585] [sirius-web] Additional foreign keys set up between bounded contexts have been deleted.
These should not have existed in the first place and have now been replaced with transaction event listeners.
As a consequence, additional domain events can now be fired while using Sirius Web giving downstream projects additional insights into the consequences of an action.
- https://github.com/eclipse-sirius/sirius-web/issues/5502[#5502] [sirius-web] Align the identity of representation content to match the one of representation metadata.
As a result, creating representation content will now require the `semantic_data_id` just like creating representation metadata.
On top of that, the link between representation content and representation metadata is now more explicit with a proper column `representation_content#representation_metadata_id` to link the two.
- https://github.com/eclipse-sirius/sirius-web/issues/6013[#6013] [sirius-web] We will now request the `editingContextId` for the deletion of representations.
We could previously derived this piece of information from the id of the representation but it was quite different from every other mutation and with the change to the internal identity of representation it would not work anymore.
Methods like `IRepresentationApplicationService#findEditingContextIdFromRepresentationId`, `IRepresentationMetadataSearchService#findSemanticDataByRepresentationId` or `IRepresentationMetadataRepository#findSemanticDataIdFromRepresentationId` have thus been removed since they will not make any sense in the very near future.
Sirius Web will not be able to figure out the context in which you are working from using only the id of a representation, you will have to provide the editing context id just like for any other operation.
- https://github.com/eclipse-sirius/sirius-web/issues/6014[#6014] [sirius-web] Ask for the semantic data id anytime we manipulate representation metadata.
This will help align the manipulation of representation on the manipulation of other objects since the semantic data id will be available.
Multiple methods used to manipulate representation metadata will now either require the semantic data id (in Sirius Web modules) or editing context id (in the Sirius Components modules).
This will mainly affect `IRepresentationMetadataSearchService`, `IRepresentationMetadataUpdateService` and `IRepresentationMetadataDeletionService`, which will require an argument `AggregateReference<SemanticData, UUID> semanticData`, and services calling them.
* `IRepresentationSearchService#existByIdAndKind` will now require the `IEditingContext``
* `IRepresentationMetadataProvider#getMetadata` will now require the `editingContextId`
* `IRepresentationApplicationService#findRepresentationMetadataById` will not require the `editingContextId`
- https://github.com/eclipse-sirius/sirius-web/issues/6014[#6014] [sirius-web] Ask for the semantic data id anytime we manipulate representation content.
Methods from `IRepresentationContentSearchService`, `IRepresentationContentCreationService` and `IRepresentationContentUpdateService` will require an argument `AggregateReference<SemanticData, UUID> semanticData` along with a `AggregateReference<RepresentationMetadata, UUID> representationMetadata`.
This will help distinguish the various `UUID` which may be required along with the expected value for those `UUID`.
- https://github.com/eclipse-sirius/sirius-web/issues/5502[#5502] [sirius-web] Leverage the semantic data id anytime we need to manipulate representation metadata or content.
The identifier of both representation metadata and representation content have been modified from a UUID to a string.
The value of this string is currently `SEMANTIC_DATA_ID#REPRESENTATION_METADATA_ID`.
Once should not make any hypothesis on the shape of this new identifier.
Do not try to parse it or leverage it at all.
This new identifier is also used by `representation_metadata_icon_url#representation_metadata_id`.
The previous value of `representation_metadata#id` can now be found in `representation_metadata#representation_metadata_id`.
These changes have been introduced due to a limitation of Spring Boot 3.x, once we will upgrade to Spring Boot 4.x, we will move away from this hack in favor of a proper composite identifier involving `representation_metadata#id` and `representation_metadata#semantic_data_id`.
- https://github.com/eclipse-sirius/sirius-web/issues/6022[#6022] [sirius-web] The "Representations" view has been renamed to "Related Views" to better reflect its purpose.
The GraphQL subscription `representationsEvent` has been renamed to `relatedViewsEvent`, and the input type `RepresentationsEventInput` has been renamed to `RelatedViewsEventInput`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5843[#5843] Add range selection (SHIFT + click) for tree view's items.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5908[#5908] [diagram] Prevent border node to be detached from its parent after resize
- https://github.com/eclipse-sirius/sirius-web/issues/5931[#5931] [diagram] Prevent label resize handles from being visible when labels are hidden
- https://github.com/eclipse-sirius/sirius-web/issues/5975[#5975] [diagram] Prevent newly created nodes from stacking in the same position
- https://github.com/eclipse-sirius/sirius-web/issues/6048[#6048] [sirius-web] Display the default workbench configuration for libraries.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6029[#6029] [diagram] Fix a potential infinite loop when exporting some diagrams as SVG
- https://github.com/eclipse-sirius/sirius-web/issues/6089[#6089] [diagram] Prevent delete from diagram tool from crashing when the node has no inside label
- https://github.com/eclipse-sirius/sirius-web/issues/6069[#6069] [sirius-web] Fix an issue preventing some letters from being typed in the duplicate modal
- https://github.com/eclipse-sirius/sirius-web/issues/6058[#6058] [diagram] Fix an issue where edge `reset handle quick tool` could be available even if the handle was not moved.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5810[#5810] [sirius-web] Fix an issue where the out-to-date representations were not migrated to the latest version.
- https://github.com/eclipse-sirius/sirius-web/issues/6228[#6228] [diagram] Prevent list nodes from being dropped at invalid positions
- https://github.com/eclipse-sirius/sirius-web/issues/5993[#5993] [diagram] Prevent border nodes from changing position when resizing or moving the parent
...
- https://github.com/eclipse-sirius/sirius-web/issues/5782[#5782] [view] Add the concept of GroupPalette in the diagram view dsl to provide tools to execute while multiple
...
- https://github.com/eclipse-sirius/sirius-web/issues/5955[#5955][view] When using a _Let_ operation inside a tool definition, the corresponding binding is now made available in the tool's _Elements to Select Expression_.
This can be used for example to capture semantic elements created (or simply retrieved) by arbitrary Java services (by wrapping a _Change Context_ operation invoking the service into a _Let_) to be then used in the _Elements to Select Expression_.
If multiple _Let_ operations are executed inside the body of a tool with the same variable name, only the last executed one is available.
- https://github.com/eclipse-sirius/sirius-web/issues/5873[#5873] [diagram] Add the ability to hide the mini map.
This preference is saved in the browser local storage.
https://github.com/eclipse-sirius/sirius-web/issues/5782[#5782] [diagram] Add the ability to execute tools on a selection of multiple elements
...
- https://github.com/eclipse-sirius/sirius-web/issues/4348[#4348] [diagram] Add `IGroupPaletteToolsProvider` in order to provide extra tools to a group palette
- https://github.com/eclipse-sirius/sirius-web/issues/5585[#5585] [sirius-web] Replace use of foreign keys in project semantic data with proper events
- https://github.com/eclipse-sirius/sirius-web/issues/5752[#5752] [table] Add the functionality to expand all row in one action
- https://github.com/eclipse-sirius/sirius-web/issues/5863[#5863] Add `toolSections` to groupPalette in the view dsl.
- https://github.com/eclipse-sirius/sirius-web/issues/5934[#5934] [diagram] Support multiple elements with diagram appearance tools
- https://github.com/eclipse-sirius/sirius-web/issues/5893[#5893] [diagram] Add a new quick tool to reset label resizing
- https://github.com/eclipse-sirius/sirius-web/issues/5765[#5765] [diagram] Add the possibility to interpret expressions in the style of a node.
An example has been added in Sirius-Web with the new _opacity_ field in the custom _ellipse_ node.
- https://github.com/eclipse-sirius/sirius-web/issues/6004[#6004] [sirius-web] Lower the coupling between `CreateProjectInput` and its data fetcher.
Event listeners reacting to a event caused by a `CreateProjectInput` should instead rely on `ICreateProjectInput`.
- https://github.com/eclipse-sirius/sirius-web/issues/4860[#4860] [core] The workbench is now configured by default by the backend.
The default implementation of `IWorkbenchConfigurationProvider` delegates to any implementation of `IWorkbenchConfigurationProviderDelegate`.
If none of the provided implementation of `IWorkbenchConfigurationProviderDelegate` is able to handle the `editingContextId`, then returns the default workbench configuration provided by the implementation of `IDefaultWorkbenchConfigurationProvider`.
With this architecture, downstream projects can customize this behavior by providing an implementation of `IWorkbenchConfigurationProviderDelegate` to provide new workbench configuration for their needs, or replace the default implementation of `IDefaultWorkbenchConfigurationProvider` to change the default workbench configuration.
With the workbench configuration, downstream projects can give the representation to open when the user open a project and in which side panel views will be available.
In conjunction of the `workbenchViewContributionExtensionPoint` frontend extension point, downstream projects can add custom views with their workbench configuration.
- https://github.com/eclipse-sirius/sirius-web/issues/5815[#5815] [diagram] Highlight the last selected elements with custom resize handles, this can be useful when executing a layout group tool.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5656[#5656] [diagram] Add undo redo for edge layout and appearance
...
- https://github.com/eclipse-sirius/sirius-web/issues/5522[#5522] [core] Provide a search history in omnibox command.
- https://github.com/eclipse-sirius/sirius-web/issues/5966[#5966] [diagram] The UI of the palette has been tweaked to reduce the risk of misclicks on quick actions
- https://github.com/eclipse-sirius/sirius-web/issues/5813[#5813] Add new project setting to enable/disable confirmation modals.
- https://github.com/eclipse-sirius/sirius-web/issues/5977[#5977] [sirius-web] Leverage `SemanticDataDeletedEvent` to delete Elasticsearch indices.
- https://github.com/eclipse-sirius/sirius-web/issues/5715[#5715] [table] Add `hasChildrenExpression` on `RowDescription` to block the expand action on rows that have no children
- https://github.com/eclipse-sirius/sirius-web/issues/5939[#5939] [diagram] When moving a node on a diagram, we now _highlight_ compatible drop targets/locations instead of fading all the incompatible ones
- https://github.com/eclipse-sirius/sirius-web/issues/5884[#5884] [diagram] Facilitate the moving of a border node even when an edge is selected
- https://github.com/eclipse-sirius/sirius-web/issues/5989[#5989] [sirius-web] Add button to access explorer filter bar feature
- https://github.com/eclipse-sirius/sirius-web/issues/5991[#5991] [diagram] Improve the UI of the Manage Visibility modal
...
- https://github.com/eclipse-sirius/sirius-web/issues/5673[#5673] [emf] Both `IRepresentationDescriptionConverter` and `IViewConverter` convert methods now return `ViewConverterResult` and a `list of ViewConverterResult`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5762[#5762] [diagram] Fix an issue where the selection requested by a diagram tool was no longer applied globally to the workbench
...
- https://github.com/eclipse-sirius/sirius-web/issues/6009[#6009] [explorer] Fix an issue where the Explorer view filter bar research introduced unwanted characters in Explorer view tree items labels.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5215[#5215] [diagram] Add the ability to programmatically change the layout of a diagram on the backend
...
- https://github.com/eclipse-sirius/sirius-web/issues/5334[#5334] [sirius-web] Fix the capability name to list libraries to `Library:list`.
...
This occurred for the tree filter menu in the Explorer when a table was opened, and with the diagram filters menu which could be obscured by the right-side panel.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5140[#5140] [diagram] Add support for the management of the appearance of custom nodes with the extension point, `PaletteAppearanceSectionContribution`, an example is available on ellipse node.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5447[#5447] [diagram] Removed some unnecessary animations on the diagram, which no longer zooms/pans on newly created or selected node if they are already fully visible.
...
Of particular note: projects which create View-based, in-memory, representation description programmatically and rely on `EcoreUtil.getURI()` to derive a stable object id should either:
...
- https://github.com/eclipse-sirius/sirius-web/issues/6492[#6492] [sirius-web] Only execute the Explorer's Duplicate object action when invoked from the Explorer view
...
- https://github.com/eclipse-sirius/sirius-components/issues/562[#562] [graphql] Move the `representation` and `relatedviews` fields from `Project` to `EditingContext`
